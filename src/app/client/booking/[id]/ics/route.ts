/**
 * Route publique GET /{slug}/client/booking/{id}/ics
 *
 ...
 * que le client connaît déjà (sa propre réservation). On vérifie quand même
 * que le booking appartient bien au tenant courant (header middleware) pour
 * éviter les enumeration cross-tenant via UUID guessé.
 *
 * Headers :
...
export async function GET(
  ...
  if (!tenantId) {
    return new Response('Tenant context missing', { status: 400 });
  }
  ...
  const bookingRes = await admin
    .from('bookings')
    .select(
      'id, tenant_id, starts_at, ends_at, status, client_display_name, services(name), staff(name)',
    )
    .eq('id', bookingId)
     // garde tenant — empêche enumeration cross-tenant
    .maybeSingle();
  ...
  const location = h.get('x-tenant-slug') ?? tenantName;

  // VCALENDAR / VEVENT minimal RFC 5545. PRODID identifie System A.
  // UID = booking.id pour qu'un re-import fasse un update et pas un duplicate.
  const lines = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//System A One//Client Booking//FR',
    'CALSCALE:GREGORIAN',
    'METHOD:PUBLISH',
    'BEGIN:VEVENT',
    `UID:${booking.id}@system-aone.com`,
